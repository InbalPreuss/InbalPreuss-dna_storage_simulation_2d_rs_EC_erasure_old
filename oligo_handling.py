...
class OligoHandling:
    def __init__(self, a_number_of_barcode_letters, a_file_name):
        oligo_input_file_name = glob.glob(a_file_name + '*')
        ...
        if oligo_input_file_name.__len__() == 0:
            ...
            raise NameError('There are too many files with the same name as ' + a_file_name + ', please make sure you '
    ...
    def parse_oligo(self):
        try:
            file_type = FileType.FileType(self.file_name, self.file_extension)
        ...
        # oligo_input_file_barcode_df = oligo_input_file_df.join(oligo_input_file_df['oligo_dna_sequence'].str[:self.number_of_barcode_letters].to_frame(), rsuffix='_barcode')
