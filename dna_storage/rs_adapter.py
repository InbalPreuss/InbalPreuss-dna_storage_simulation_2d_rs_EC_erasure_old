...
import numpy as np
from unireedsolomon.unireedsolomon import rs, RSCodecError, ff
...
class RSPayloadAdapter:
    ...
    def decode(self, payload_encoded, erasures_positions):
        ff.set_globals(*self.ff_globals)
        ...
        if self._payload_coder.check_fast(payload_as_int):
            ...
            try:
                payload_as_gf, rs_as_gf = self._payload_coder.decode(payload_as_int, erasures_pos=erasures_positions, nostrip=True, return_string=False)
            except RSCodecError:
                return payload_encoded[0:self.payload_len]
